...
export function renderConfigErrorScreen(reason: string) {
  ...
  if (root) root.innerHTML = ERROR_HTML;
}
...
export function checkSupabaseEnv(): boolean {
  ...
  renderConfigErrorScreen(`Variáveis ausentes no bundle: ${missing.join(", ")}`);
  return false;
}

/**
 * Install global handlers to catch the synchronous "supabaseUrl is required"
 * error that fires from lazy chunks when env vars are missing in the bundle.
 * Without this, the lazy chunk error happens AFTER checkSupabaseEnv() passed
 * (because env vars exist in dev but not in publish build) → white screen.
 */
export function installSupabaseErrorHandler() {
  if (typeof window === "undefined") return;

  const SUPABASE_ERR_RE =
    /supabaseUrl is required|supabaseKey is required|Invalid URL/i;

  const handle = (msg: string) => {
    if (SUPABASE_ERR_RE.test(msg)) {
      renderConfigErrorScreen(msg);
    }
  };

  window.addEventListener("error", (e) => {
    handle(String(e?.message || e?.error?.message || ""));
  });

  window.addEventListener("unhandledrejection", (e) => {
    const reason: any = e?.reason;
    handle(String(reason?.message || reason || ""));
  });
}
